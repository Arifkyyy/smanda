...
export function Card({ children, className = '' }: { children: ReactNode; className?: string }) {
  return <div className={`bg-white rounded-2xl border border-slate-100 shadow-soft ${className}`}>{children}</div>;
}
...
export function StatCard({ icon: Icon, label, value, hint, tone = 'purple' }: { icon: LucideIcon; label: string; value: string; hint?: string; tone?: Tone }) {
  return (
    <Card className="p-5">
...
const statusStyles: Record<string, string> = {
  ...
  Complete: 'bg-emerald-50 text-emerald-600 ring-emerald-200',
...
export function StatusPill({ status }: { status: string }) {
  ...
  return <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ring-1 ${cls}`}>{status}</span>;
}
